...
<div class="h-full flex flex-col items-center divide-y-8">
	<div
		class="bg-img w-full h-2/6 md:px-12 sm:px-4 px-2 flex flex-col transition-[height] duration-500 ease-in-out"
		style={bgImg}
		bind:this={bannerContainer}
	>
		<div
			class="outline bg-white bg-opacity-40 h-1/2 my-auto flex items-center w-full justify-between transition-[outline] duration-750 ease-out"
			bind:this={banner}
		>
	...
	<div
		class="w-full max-w-5xl h-4/6 overflow-y-scroll"
		on:scroll={bannerController}
		bind:this={content}
	>
		<slot name="content" />
	</div>
